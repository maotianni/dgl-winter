...
def future_task(n_epochs, batch_size, batch_size_test, use_cuda, gpu,
    ...
    for epoch in range(n_epochs):
        ...
        if epoch % eval_every == 0:
            ...
            time_ev_0 = time.time()
            if validation:
                node_cls_res = utils.node_class(model, nc, utils.Sample(g_v, num_negs=num_neg), n_train,
                ...
                if epoch > 0 and node_cls_res['AUC'] > eval_res['best_eval_auc']:
                    ...
                    print('Testing...')
                    test_res = utils.node_class(model, nc, utils.Sample(g_t, num_negs=num_neg), n_train+n_val,
                ...
                node_cls_res = utils.node_class(model, nc, utils.Sample(g_t, num_negs=num_neg), n_train,
